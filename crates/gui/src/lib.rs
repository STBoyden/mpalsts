...
use mpalsts_shared::{
	AppState, DEFAULT_LUMENS_THRESHOLD, DEFAULT_SECONDS_THRESHOLD, MAX_LUMENS_THRESHOLD,
...
struct App {
	...
	light_sensor: Entity<Rc<mpalsts_sensors::ConcreteSensor>>,
...
impl App {
	fn new(persistent_state: Entity<AppState>, window: &mut Window, cx: &mut Context<Self>) -> Self {
		...
		let light_sensor_rc = Rc::new(mpalsts_sensors::get_platform_reader());
      ...
      if !this.persistent_state.read(cx).enable_theme_switching {
		...
		let theme_switcher = cx.new(|_| mpalsts_theme_switch::get());
